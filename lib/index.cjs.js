...
function defaultFormatter(scaleType) {
  if (scaleType === 'time') {
    return d3TimeFormat.timeFormat('%Y-%m-%d');
  }
  return v => {
...
class AxisController extends EventEmitter.EventEmitter {
  /**
   * @param {object} opts
  ...
  scaleDomainFromMidpoint(factor) {
    ...
    if (this.scaleType === 'log') {
      ...
      this.setDomain([Math.pow(10, logMid - newHalf), Math.pow(10, logMid + newHalf)]);
    } else {
      const mid = (min + max) / 2;
  ...
  panByPixels(pixelDelta) {
    ...
    if (this.scaleType === 'log') {
      ...
      this.setDomain([Math.pow(10, logMin + dataDelta), Math.pow(10, logMax + dataDelta)]);
    } else {
...
const PlotCanvas = /*#__PURE__*/React.forwardRef(function PlotCanvas(props, ref) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: modeStyle
  }, modeText), /*#__PURE__*/React.createElement("div", {
    style: overlayStyle
