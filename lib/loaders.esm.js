...
import { getTableLength, getTableRowAsObject, getTableCell } from '@loaders.gl/schema';
...
class TableLoaderAdapter extends EventEmitter {
  /**
  ...
  async _process(arrayBuffer, loader) {
    ...
    while (loaded < total) {
      ...
      if (typeof this._sizeOpt === 'string' && columns.includes(this._sizeOpt)) {
        ...
        for (let i = 0; i < count; i++) size[i] = this._toFloat(getTableCell(table, loaded + i, this._sizeOpt), this._sizeOpt, loaded + i);
...
class RasterLoaderAdapter extends EventEmitter {
  /**
  ...
  async _loadNetCDF(arrayBuffer) {
    let parsed;
    try {
      parsed = await parse(arrayBuffer, NetCDFLoader, {
        netcdf: {
          loadData: true
        }
      });
  ...
  _registerLayer({
    ...
    this._ctrl.registerDataLayer(this._layerId, () => new BitmapDataLayer({
      id: this._layerId,
      source,
      width,
      height,
...
export { RasterLoaderAdapter, TableLoaderAdapter };
//# sourceMappingURL=loaders.esm.js.map
