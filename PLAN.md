...
### F41 [PENDING] Configurable Keybindings (ROI Creation + Zoom/Pan + Scale Presets)

**Branch:** `feature/F41`
**Depends on:** none (independent of F39/F40/EX22)

**Problem:** `ROIController._onKeyDown` (`src/plot/ROI/ROIController.js:304-335`) hardcodes `d`/`l`/`r`/`v`/`h`/`Escape`; its constructor takes no `opts`. `PlotController` has exactly one configurable key (`autoScaleKey`, F23), no keyboard zoom-in/out, arrow-key panning, or preset-view jump.

**New files:** `test/plot/ROI/ROIController.test.js`
**Modified:** `src/plot/ROI/ROIController.js`, `src/plot/PlotController.js`, `src/index.d.ts`, `test/plot/PlotController.test.js`, `examples/ExampleApp.jsx`, `README.md`, `examples/docs/ApiReferencePage.jsx`

**Keybindings (`ROIController` + `PlotController`):**
- `ROIController` constructor becomes `(viewport, opts = {})` with `opts.keyBindings` scoped to `createLinear`(`l`)/`createRect`(`r`)/`createVLine`(`v`)/`createHLine`(`h`)/`deleteROI`(`d`)/`cancel`(`escape`) — defaults unchanged. New `setKeyBindings(patch)` for runtime remap; validate/warn/fallback + same-key-collision warning, mirroring F38's `_setMouseButtonMap` precedent. `deleteROI` keeps firing regardless of mouse-over (existing behavior); others stay gated on `_mouseIsOver`.
- `PlotController` unifies all keybindable actions under one `opts.keyBindings`, adding `autoScale`(`' '`), `zoomIn`(`'='`), `zoomOut`(`'-'`), `panLeft`(`arrowleft`), `panRight`(`arrowright`), `panUp`(`arrowup`), `panDown`(`arrowdown`) alongside the ROI actions (forwarded to `ROIController`). `opts.autoScaleKey` is **kept working as a deprecated alias** (maps into `keyBindings.autoScale` with a one-time warn, unless `keyBindings.autoScale` was also explicitly given) — not removed, since 1.0.0 is already published and this must stay backward-compatible. New `setKeyBindings(patch)` runtime setter mirroring `setMouseButtons()`.
- `zoomIn`/`zoomOut` call `viewport.scaleDomainFromMidpointX/Y(factor)` (existing F21 methods; factor `1.25` in / `0.8` out). `panLeft/Right/Up/Down` call `viewport.panByPixels({dx,dy})` with a 40px step. **Locked-in signs** (camera pans toward the arrow, matching F5's follow-pan precedent, re-derived from `ViewportController._panDomain`'s actual formula per the corrected prompt.md convention above): `panRight → dx:-40`, `panLeft → dx:+40`, `panUp → dy:+40`, `panDown → dy:-40`. Gated behind `!this._disablePanZoom` (ROI keys stay ungated, unaffected by that flag, matching current semantics).

**Scale presets (new):** press a bound key to jump to a fixed view — e.g. `'1'` → y snaps to `[0,200]`, x left as-is.
- New `opts.scalePresets`: array, no defaults (`[]`), fully opt-in — `[{ bind: '1', yMin: 0, yMax: 200 }, { bind: '2', xMin:0, xMax:10, yMin:-5, yMax:5 }]`. Either axis pair is optional; if present, both halves must be present, finite, and `min !== max` (reusing REL7's `_validateDomain` precedent) — invalid entries warn + are skipped. Duplicate `bind` across presets → warn, last wins.
- New `setScalePresets(presets)` — **replaces** the whole array (not a merge/patch, unlike `setKeyBindings`/`setMouseButtons` — document this difference).
- `_onKeyDown`, same `!disablePanZoom` gate: look up the pressed key in a prebuilt `Map`; for each axis present in the matched preset, override that domain, otherwise keep `viewport.getXDomain()`/`getYDomain()`'s current value; apply atomically via `viewport.setDomains(xDomain, yDomain)`.
- **Documented caveat:** preset keys are checked for collisions against each other but not against `keyBindings` (separate listener for ROI actions, separate lookup table for zoom/pan actions) — a preset bound to `'l'` would fire alongside ROI's `createLinear`. Recommend digits (`1`–`9`) in docs as the natural collision-free convention.

**Verification:** `npm test` (new `ROIController.test.js` — `init()` only attaches DOM listeners, no WebGL needed, so a plain `document.createElement('canvas')` works in jsdom; plus `PlotController.test.js` additions for `keyBindings`/`scalePresets` validation and the `autoScaleKey` alias path). `npm run build` zero errors. Manual browser verification that default behavior is unchanged, remapping works, and zoom/pan/preset keys feel directionally correct (interaction change — can't be fully exercised in jsdom since `init()` needs real WebGL).
