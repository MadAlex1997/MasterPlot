...
**Status:** Phase 7 + Phase 8 + Phase 9 + Phase 10 (1.0.0 Release Hardening) complete — v1.0.0 published, currently at v1.0.2 on npm. Phase 11 (High-Precision Time Axes & Configurable Keybindings) complete — F39/F40/EX22 and F41 both merged to main.
...
| F41 | Configurable Keybindings (ROI + Zoom/Pan + Scale Presets) | ✅ COMPLETED | feature/F41 | 2026-08-05 |
...
**Completed:** 2026-07-25 | **Branch:** feature/REL7-REL8
Extended F38's warn+fallback precedent across the public surface: `PlotController` constructor options (`xDomain`/`yDomain`/`xScaleType`/`yScaleType`/`panMode`) warn and fall back to their documented defaults on malformed input; `DataStore.appendData()` throws naming the offending field for missing/mismatched `x`/`y`/`size`/`color`; `ExternalDataAdapter`/`ExternalROIAdapter` constructors now validate required method overrides at registration time (construction) rather than first use; `ROIController.updateFromExternal()` validates serialized-ROI shape (id/type/version/domain) before version-gating, warning and rejecting (not mutating existing state) on malformed input. 30 new tests across 5 files; 136/136 passing.
Full spec: [docs/plan-archive.md#rel7](docs/plan-archive.md#rel7)

---

### REL8 [COMPLETED] Peer Dependency Range Audit
**Completed:** 2026-07-25 | **Branch:** feature/REL7-REL8
Added a tested upper bound to every `peerDependencies` entry (e.g. `"@deck.gl/core": ">=9 <10"`), based on versions actually installed in this repo (deck.gl/luma.gl 9.x, React 19.2.4, d3-scale 4.x, etc. — `zstd-codec` capped at the next minor since it's pre-1.0). Regenerated `package-lock.json` (`--package-lock-only`, no dependency-tree changes). Added a "Peer Dependencies" README subsection documenting the policy and the tested-version table.
Full spec: [docs/plan-archive.md#rel8](docs/plan-archive.md#rel8)

---

### REL9 [COMPLETED] Cut & Publish v1.0.0
**Completed:** 2026-07-25 | **Branch:** feature/REL9
Dropped stale "prototype" framing from HubPage/ArchitecturePage (the last blocker before publish); `npm publish` ran with maintainer go-ahead; `v1.0.0` tagged. Two small follow-up fixes shipped afterward outside the tracked feature workflow, published as `v1.0.2` (no separate `v1.0.1` tag exists). Currently published version: 1.0.2.
Full spec: [docs/plan-archive.md#rel9](docs/plan-archive.md#rel9)

---

## Phase 11 — High-Precision Time Axes & Configurable Keybindings

**Motivation:** Three gaps surfaced from real usage on top of the published 1.0.x baseline.
(1) `AxisController`'s time-scale formatter is hardcoded to `%Y-%m-%d`, so tick labels don't
adapt when zooming from a multi-year span down to sub-second — no example currently uses
`scaleType:'time'`, but the formatter needs fixing before anyone can. (2) There's no supported
way to plot high-precision epoch timestamps: `DataStore`'s GPU buffers are `Float32Array`
(~7 significant digits), which can't hold an epoch-seconds value with microsecond precision —
at that magnitude float32's representable gap already exceeds 100, aliasing point positions,
not just losing sub-second precision. (3) ROI-creation keys (`l`/`r`/`v`/`h`/`d`/`Escape`) are
hardcoded in `ROIController`, and there's no keyboard zoom-in/out, arrow-key panning, or
preset-view jump (e.g. "press 1 to snap y to a known sensor range") at all today. All three
are backward-compatible additions — no major version bump required.
...
**Completed:** 2026-08-05 | **Branch:** feature/F41
`ROIController` constructor is now `(viewport, opts = {})` with configurable `keyBindings` + `setKeyBindings()`; `PlotController` unifies ROI-creation actions and new zoom/pan actions (`autoScale`/`zoomIn`/`zoomOut`/`panLeft`/`panRight`/`panUp`/`panDown`) under one `keyBindings` option mirroring F38's `mouseButtons` pattern, with `autoScaleKey` kept as a deprecated alias; new opt-in `scalePresets` array binds a key to a fixed view-jump on one or both axes via `setDomains()`. 171/171 tests passing.
Full spec: [docs/plan-archive.md#f41](docs/plan-archive.md#f41)
...
- **2026-08-05 [Claude]**: `feature/F39-F40-EX22` and `feature/F41` merged to `main`, completing Phase 11 (both branches were developed independently and completed the same day — see their individual entries below for full detail). Merge conflicts were limited to documentation (`PLAN.md`, `README.md`, `docs/plan-archive.md`) plus two `lib/*.map` source-map files and `test/plot/PlotController.test.js`'s two independent new `describe` blocks; all resolved by keeping both sides' additions (no functional overlap between the two branches' changes) and rebuilding `lib/` fresh via `npm run build` rather than hand-merging the generated map files. Full suite verified post-merge.
- **2026-08-05 [Claude]**: F41 completed (v9.23), on `feature/F41` (branched independently from `feature/F39-F40-EX22`, per the mandatory-order note that F41 has no dependency on F39/F40/EX22). `ROIController` constructor is now `(viewport, opts = {})` accepting `opts.keyBindings` (partial override of the ROI-creation action→key map, defaults unchanged: `l`/`r`/`v`/`h`/`d`/`escape`), plus a new `setKeyBindings()` runtime setter; validation follows F38's `_setMouseButtonMap` warn+fallback precedent, with an added same-key-collision warning and support for an explicit `null` value to disable a given action's key. `PlotController` unifies ROI actions (forwarded to `ROIController`) and new zoom/pan actions — `autoScale`(`' '`, replacing the deprecated `autoScaleKey`, kept as a warn-once alias for backward compatibility since v1.0.2 is already published), `zoomIn`(`'='`)/`zoomOut`(`'-'`) via the existing F21 `scaleDomainFromMidpointX/Y(1.25|0.8)`, and `panLeft`/`panRight`/`panUp`/`panDown` (arrow keys) via `panByPixels()` with a 40px step — under one `keyBindings` option + `setKeyBindings()`. Arrow-key pan direction was locked to "camera pans toward the arrow" per the user's decision, requiring `prompt.md`'s Y-axis Coordinate Convention section to be corrected first (done in the prior housekeeping commit) since its old "Follow scroll" example was inconsistent with F5's actual shipped sign convention. New opt-in `scalePresets` array (no defaults) binds a key to a fixed-view jump on one or both axes, validated with REL7's `_validateDomain`-style rules and applied atomically via the existing `viewport.setDomains()`; `setScalePresets()` replaces the array rather than merging, a documented deliberate difference from `setKeyBindings`/`setMouseButtons`. New `test/plot/ROI/ROIController.test.js` (12 tests, including one exercising the real DOM `keydown` → `window.dispatchEvent` wiring path, not just direct method calls) plus additions to `test/plot/PlotController.test.js`; full suite 171/171 (up from 136 on this branch's base). `src/index.d.ts`/`test-types/smoke.tsx` cover the new surface. `README.md` (new "Configurable Keybindings"/"Scale Presets" sections, updated options/methods tables, a note on the ROI Creation table), `examples/ExampleApp.jsx` (`HelpOverlay` rows for the new zoom/pan keys), and `examples/docs/ApiReferencePage.jsx` (option/method rows + updated `ROIController` constructor callout) all updated. Verification: `npm test` (171/171), `npm run build`/`lint`/`typecheck` all clean; **no browser/screenshot tool was available in this environment to visually confirm interaction feel** (arrow-key pan direction, zoom responsiveness) — flagged rather than claimed as fully verified, matching the same limitation noted for EX22.
